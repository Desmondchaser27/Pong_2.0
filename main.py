...
paddle_left = Paddles.Paddle(10, window.get_height() / 2)
paddle_right = Paddles.Paddle(window.get_width() - 40, window.get_height() /2)
ball = Ball.Ball(window.get_height() / 2, window.get_width() / 2)
running = True
...
while running:
    ...
    window.blit(ball.ball, (window.get_width()/2, window.get_height()/2))
    ball.move()
